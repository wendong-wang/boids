...
class VICSEK():
    ...
    --
        ...
        # P: matrix of the positions of the units n x 2 x stepNum 位置矩阵
...
#================== animation part
    ...
    def start(self) -> None:
        ...
        fig = plt.figure() # 
        plt.quiver(self.P[:,0],self.P[:,1],self.V[:,0],self.V[:,1])
    ...
    def simulation_Init(self):
        ...
        # os.chdir(output_folder) 先不进入，进入后就无法进行多文件的读写操作了最后进行统一的复制
    ...
    def _move(self, frameNumber):
        ...
        plt.xlim((0, self.l))
        plt.ylim((0, self.l))
        plt.quiver(P[:,0],P[:,1],self.V[:,0],self.V[:,1])
        pass
    ...
    def update(self):
        ...
        '''
        dx = np.subtract.outer(self.P[:, 0], self.P[:, 0])
        dy = np.subtract.outer(self.P[:, 1], self.P[:, 1]) 
        ...
        self.Theta = np.arctan2(np.matmul(WxA,self.V[:,1].reshape(self.n,1)),\
                                np.matmul(WxA,self.V[:,0].reshape(self.n,1))+0.000000000001) 
        ...
        self.V = np.hstack((self.v*np.cos(self.Theta),self.v*np.sin(self.Theta)))
        # print(self.V)
        self.P  = self.P + self.V * 1 # 1 means time
        # print(self.P) 
        self.P = np.mod(self.P, self.l) # 取余
...
[3] Modes of information flow in collective cohesion
'''
